...
print('   Computing Curlometer recon')
...
for t_ind in range(t_steps):
    ...
    # Curlometer reconstruction of B field 
    ...
    for k in range(N_timeseries):
        ...
        for j in np.where(ind_good == True)[0]:
            if ind_good[j] == True:
                ...
                index = list(poly_indices[j])
                DivB_temp[j], B_temp[j,:], CurlB_temp[j,:] = Curlometer3D(r_sc_taylor[:,index].T,B_sc_taylor[:,index].T,X0)
                tetra_passed[k] += 1
...
ax[1].set_title('$B_x$ of Curlometer Reconstructed Field', fontsize=18)
