...
def E_func(sigma, r_sc_rbf, B_sc_rbf):
    N_pts = np.shape(r_sc_rbf)[1]
    L = np.shape(r_sc_rbf)[0]
    #print('sigma=%.2f, N=%i, L=%i' %(sigma, N_pts, L))
    E = np.zeros([N_pts,L])
    for i in range(N_pts):
        r_used = np.hstack((r_sc_rbf[:,:i], r_sc_rbf[:,i+1:]))
        B_used = np.hstack((B_sc_rbf[:,:i], B_sc_rbf[:,i+1:]))
        
        rbf_fn = Rbf(r_used[0,:], r_used[1,:], r_used[2,:],  B_used.T, mode='N-D', function=method, epsilon=sigma)
        B_recon_rbf = rbf_fn(r_sc_rbf[0,i], r_sc_rbf[1,i], r_sc_rbf[2,i]).T
        B_true = B_sc_rbf[:,i]
        E[i,:] = B_true - B_recon_rbf
    #print('   RMS error = %.5f' %la.norm(E))
    return la.norm(E)

# %% Configurable parameters
SAVE_figs = True
fig_path = 'figures/RBF'
B_data_path = 'sc_data'
method = 'multiquadric'
...
#r is a [3 x num_sc] array of spacecraft locations in 3D space
#r_recon is a [n x 3] array of locations where we wish to reconstruct the vector field
#B is a [3 x num_sc] array of the vector field measurements
#sigma is the standard deviation of the RBF kernal functions
r_sc_rbf = np.reshape(np.transpose(r_sc,(1,0,2)),(3,N*t_steps))
B_sc_rbf = np.reshape(np.transpose(B_sc,(1,0,2)),(3,N*t_steps))
r_recon_rbf = np.reshape(np.transpose(r_field,(1,0,2)),(3,N_timeseries*t_steps))

# %% compute optimal sigma for RBF using Rippa's (1999) algorithm
if N*t_steps > 500:
    rand_ints = np.unique(np.random.randint(0,t_steps,int(500/N)))
    r_sc_use = r_sc[:,:,rand_ints]
    B_sc_use = B_sc[:,:,rand_ints]
    r_sc_use = np.reshape(np.transpose(r_sc_use,(1,0,2)),(3,N*np.size(rand_ints)))
    B_sc_use = np.reshape(np.transpose(B_sc_use,(1,0,2)),(3,N*np.size(rand_ints)))
else:
    r_sc_use = r_sc_rbf
    B_sc_use = B_sc_rbf
print('   computing optimal RBF sigma...')
out = minimize(E_func, 100, (r_sc_use, B_sc_use), method='CG')
sigma = out.x[0]

print('   Rippa sigma* = %.1f km' %sigma)
# %% Compute RBF interpolation
rbf_fn = Rbf(r_sc_rbf[0,:], r_sc_rbf[1,:], r_sc_rbf[2,:], B_sc_rbf.T, mode='N-D', function=method, epsilon=sigma)
B_recon_rbf = rbf_fn(r_recon_rbf[0,:], r_recon_rbf[1,:], r_recon_rbf[2,:]).T
B_recon = np.transpose(np.reshape(B_recon_rbf,(3,N_timeseries,t_steps)),(1,0,2))
...
ax[1].set_title('$B_x$ of RBF Reconstructed Field', fontsize=18)
